  ...
  <div >
    <div>
      ...
      <el-container style="height: 600px; border: 5px solid #eee">
        <el-header  style="background-color: rgb(72,209,204)">
            <p6 style="margin-left: 10px;height: 500px;font-size:22px;">后台管理审核系统</p6>
            <el-button plain round icon="el-icon-view" type="success" style="margin-left: 20px" @click="usermanage">用户注册审核</el-button>
            <el-button plain round icon="el-icon-view" type="success" @click="shopmanage" >商家注册审核</el-button>
            <el-button plain round icon="el-icon-view" type="success" @click="goodmanage" >上架商品审核</el-button>
        </el-header>
        <el-main style="height:0;flex-grow:1;">
            <el-col :span="3">
            <el-input style="margin-left: 10px" width="10" v-model="search" v-on:input ="inputFunc" clearable="true" placeholder="输入关键字搜索"/>
            </el-col>
            ...
            @row-click="openDetails" border>
                ...
                <el-table-column  width="160" label="操作">
                    <template slot-scope="scope">
                        <el-button size="mini" plain icon="el-icon-plus" type="info" @click.native.prevent="deleteRow(scope.$index, tableData)">允许注册</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </el-main>
      ...
      this.$axios.post('http://localhost:8080/getshopzhuce', formData, config)
        .then(res => {
          ...
          for (var i = 0; i < res.data.length; i++) {
            this.tableData[i].licence = require('C:/Users/zzw12/Pictures/shop/' + this.tableData[i].licence)
            this.tableData[i].card = require('C:/Users/zzw12/Pictures/shop/' + this.tableData[i].card)
          }
